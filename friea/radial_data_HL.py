...
def main():
    ...
    transition_data = pd.read_excel('./transition_data/back_transition_data.ods',engine='odf')
    transition_data = transition_data.set_index('ShotNumber')
    client = pyuda.Client()
    signals = {}
    ...
    for shot in transition_data.index:
        ...
        HLtime = transition_data.loc[shot]['HL']

        timeIndices = np.where(np.logical_and(
            time >= HLtime-0.1, time <= HLtime+0.1
        ))

        timeIndices = np.ravel(timeIndices)

        ThomTime.loc[:,'t'] = np.around(time[timeIndices],6)
        ThomTime.to_csv(f'./radialData_HL/{shot}/ThompsonTime.csv',index=False)
        ...
        for k in timeIndices:
            ...
            Thomdf.to_csv(f'./radialData_HL/{shot}/Thompson-{round(time[k],6)}s.csv',index=False)
        ...
        CXTime.to_csv(f'./radialData_HL/{shot}/CXRSTime.csv',index=False)
        ...
        for k in timeIndices:
            ...
            CXdf.to_csv(f'./radialData_HL/{shot}/CXRS-{round(time[k],6)}s.csv',index=False)
